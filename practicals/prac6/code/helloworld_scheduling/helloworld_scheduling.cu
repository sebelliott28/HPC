...
__global__ void helloworld_blocksthreads (void) {
...
__global__ void helloworld_blockswarps (void) {

int warpid;        

if (threadIdx.x>=0 && threadIdx.x <32) warpid =0;
else if (threadIdx.x>=32 &&threadIdx.x<64) warpid =1;
else if (threadIdx.x>=64 &&threadIdx.x<96) warpid =2;
else if (threadIdx.x>=96 &&threadIdx.x<128) warpid =3;
else if (threadIdx.x>=128 &&threadIdx.x<160) warpid =4;
else if (threadIdx.x>=160 &&threadIdx.x<192) warpid =5;
else warpid=6;
